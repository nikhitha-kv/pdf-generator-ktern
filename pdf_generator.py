...
def fetch_pexels_image(query):
    ...
    if not pexels_key:
        print("Warning: Pexels API key not set. Skipping image fetch.", file=sys.stderr)
        return None
    ...
    try:
        ...
        print(f"Error fetching image from Pexels for query '{query}': {e}", file=sys.stderr)
    return None
...
def fetch_mermaid_flowchart(mermaid_code):
    try:
        ...
        print(f"Error generating flowchart: {e}", file=sys.stderr)
    return None
...
def generate_report_data(topic, requirements):
    ...
    for model_name in models_to_try:
        ...
        print(f"Attempting generation with model: {model_name}...", file=sys.stderr, flush=True)
        try:
            r = requests.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=payload, timeout=60)
            r.raise_for_status()
            response = r
            print(f"Successfully generated with {model_name}!", file=sys.stderr, flush=True)
            break
        except Exception as e:
            print(f"Model {model_name} failed: {e}. Trying next fallback...", file=sys.stderr, flush=True)
            last_error = e
    ...
    try:
        ...
        print("Failed to decode JSON:", response_text[:200], "...", file=sys.stderr)
...
def main():
    try:
        ...
        if not topic:
            print(json.dumps({"success": False, "error": "Topic is required"}))
            return
        ...
        if requirements.strip().startswith('{') and requirements.strip().endswith('}'):
            try:
                ...
                if isinstance(workspace_data, dict) and "sections" in workspace_data:
                    print(f"Generating Premium Enterprise PDF from workspace state: {topic}", file=sys.stderr, flush=True)
                ...
                print(f"Failed to compile workspace JSON directly: {json_err}. Falling back to AI call.", file=sys.stderr, flush=True)
                
        print(f"Generating report for: {topic}", file=sys.stderr, flush=True)
        ...
        print(f"Error during generation: {str(e)}", file=sys.stderr, flush=True)
        import traceback
